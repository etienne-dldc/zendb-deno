...
export type DatatypeParsed<T extends Datatype> = T extends DatatypeJson<
  infer Value
>
  ? Value
  : {
...
export function serializeDatatype<D extends Datatype>(
  dt: D,
  value: unknown
): unknown {
  if (dt.kind === "date") {
    return datatypeTransform.date.serialize(
      dt,
      datatypeTransform.date.validate(dt, value)
    );
  }
  if (dt.kind === "integer") {
    return datatypeTransform.integer.serialize(
      dt,
      datatypeTransform.integer.validate(dt, value)
    );
  }
  if (dt.kind === "boolean") {
    return datatypeTransform.boolean.serialize(
      dt,
      datatypeTransform.boolean.validate(dt, value)
    );
  }
  if (dt.kind === "number") {
    return datatypeTransform.number.serialize(
      dt,
      datatypeTransform.number.validate(dt, value)
    );
  }
  if (dt.kind === "json") {
    return datatypeTransform.json.serialize(
      dt,
      datatypeTransform.json.validate(dt, value)
    );
  }
  if (dt.kind === "text") {
    return datatypeTransform.text.serialize(
      dt,
      datatypeTransform.text.validate(dt, value)
    );
  }
